...
Instead of "\texttt{:e}" you can use any command that has a file name argument, the "\verb!#<2!" item works in the same place as "\texttt{%}" (current file name) and "\texttt{#}" (alternate file name).
...
More info at |\texttt{:h :oldfiles}|, |\texttt{:h v:oldfiles}| and |\texttt{:h c_#<}|.
