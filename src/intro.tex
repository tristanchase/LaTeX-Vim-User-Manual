...
See |\texttt{:h vi_diff.txt}| for a summary of the differences between Vim and Vi.
...
Specifically, \texttt{time_t} and the \texttt{ctime()} function are used.
And the \texttt{time_t} is stored in four bytes in the swap file.
But that's only used for printing a file date/time for recovery, it will never affect normal editing.

The Vim \texttt{strftime()} function directly uses the \texttt{strftime()} system function.
\texttt{localtime()} uses the \texttt{time()} system function.
\texttt{getftime()} uses the time
returned by the \texttt{stat()} system function.
...
\begin{tabularx}{\textwidth}{|c|X|c|c|c|} % the 5 cm is completely arbitrary but it seems to work in the page
				...
				<EOL> & end-of-line (can be <CR>, <LF> or <CR><LF>, depends on system and \texttt{'fileformat'}) & & & \label{<EOL>}\\ %OK pour les &
...
\begin{itemize}
				...
				\item *2 Go from Visual mode to Normal mode by giving a non-movement command, which causes the command to be executed, or by hitting <Esc> "v", "V" or "CTRL-V" (see |\texttt{:h v_v}|), which just stops Visual mode without side effects.
				...
				\item *4 Go from Normal to Select mode by:
								\begin{itemize}
												...
												\item use "gh", "gH" or "g CTRL-H"  |\texttt{:h g_CTRL-H}|
								\end{itemize}
...
If the \texttt{'insertmode'} option is on, editing a file will start in Insert mode.
...
But if you use the
|\texttt{:h vimrc_example.vim}| script "Q" works like "gq".
...
If you set the \texttt{'list'} option, <Tab> characters will not be shown as several spaces, but as "\textasciicircum I".
