...
\begin{description}
				...
				\item[\{char1-char2\}]
								...
								For example: \verb!{a-z}! is a lowercase letter.
								Multiple ranges may be concatenated.
								For example, \verb!{a-zA-Z0-9}! is any alphanumeric character.
				...
				\item[CTRL-\{char\}]
								\label{CTRL-{char}}
								\verb!{char}! typed as a control character; that is, typing \verb!{char}! while holding the CTRL key down.
								The case of \verb!{char}! does not matter; thus CTRL-A and CTRL-a are equivalent.
...
\begin{tabularx}{\textwidth}{|c|X|c|c|c|} % the 5 cm is completely arbitrary but it seems to work in the page
				...
				<EOL> & end-of-line (can be <CR>, <LF> or <CR><LF>, depends on system and \verb!'fileformat'!) & & & \label{<EOL>}\\ %OK pour les &
...
This is like Normal mode, but after an operator command has started, and Vim is waiting for a \verb!{motion}! to specify the text that the operator will work on.
...
If the \verb!'insertmode'! option is on, editing a file will start in Insert mode.
...
Other non-printing characters are replaced with "\textasciicircum \verb!{char}",! where \verb!{char}! is the non-printing character with 64 added.
...
If you set the \verb!'list'! option, <Tab> characters will not be shown as several spaces, but as "\textasciicircum I".
