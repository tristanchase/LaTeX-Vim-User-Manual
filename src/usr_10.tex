...
\begin{enumerate}
				\item The "\verb!q{register}!" command starts recording keystrokes into the register named \verb!{register}.!
...
The simple form of a range is \verb!{number}!, \verb!{number}!.
For example:
...
But, instead of replacing the matched text with other text, the command \verb!{command}! is executed.
...
The command  "\verb!I{string}<Esc>!" inserts the text \verb!{string}! in each line, just left of the visual block.
...
\emph{Note}:
If you want to include characters beyond the end of the line in the block, check out the \verb!'virtualedit'! feature in chapter 25.
...
The shift amount is specified with the \verb!'shiftwidth'! option.
...
Two spaces are used after a line ending (that can be changed with the \verb!'joinspaces'! option).
...
To make this happen while inserting text, set the \verb!'textwidth'! option:
...
To check the current value of \verb!'textwidth'!:
...
In other words, it runs the system command represented by \verb!{program},! giving it the block of text represented by \verb!{motion}! as input.
...
\begin{center}\begin{longtable}{l l}
				\verb!'shell'! & specifies the program that Vim uses to execute external programs. \\
				\verb!'shellcmdflag'! & argument to pass a command to the shell \\
				\verb!'shellquote'! & quote to be used around the command \\
				\verb!'shellxquote'! & quote to be used around the command and redirection \\
				\verb!'shelltype'! & kind of shell (only for the Amiga) \\
				\verb!'shellslash'! & use forward slashes in the command (only for MS-Windows and alikes) \\
				\verb!'shellredir'! & string used to write the command output into a file \\
\end{longtable}\end{center}

On Unix this is hardly ever a problem, because there are two kinds of shells: "sh" like and "csh" like.
Vim checks the \verb!'shell'! option and sets related options automatically, depending on whether it sees "csh" somewhere in \verb!'shell'!.
