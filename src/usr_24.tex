...
In this case, it is "\texttt{ch_}", which matches with the word \texttt{ch_array}.
...
Since there is nothing in front of the cursor, it finds the first word backwards, which is "\texttt{ch_array}".
...
A fourth CTRL-P causes the editor to start over again with "\texttt{ch_array}".
...
The first match is \texttt{sub_count.c}.
This is not the one you want, so you match the next file by typing CTRL-N.
This match is \texttt{sub_done.c}.
Typing CTRL-N again takes you to \texttt{sub_exit.c}.
The results:
...
If the file name starts with \texttt{/} (Unix) or \verb!C:\! (MS-Windows) you can find all files in the file system.
...
Now you need to type the same line, but with "\texttt{s_prev}" instead of "\texttt{s_next}".
