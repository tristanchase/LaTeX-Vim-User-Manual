...
You can use \verb!|"!, this starts a new, empty command with a comment.
Example:
...
\begin{center} \begin{tabular}{l l}
				...
				\verb;-range={count}; & Range is allowed; the last number in it is used as a single number whose default is \texttt{{count}}. \\
\end{tabular} \end{center}
...
\begin{center} \begin{tabular} {l l} % This went off the page; changed to tabularx TMC
				\verb;-count={number}; & The command can take a count whose default is \texttt{{number}}.
				The resulting count can be used through the <count> keyword. \\
				\verb;-bang; & You can use a \texttt{!}.
				...
				\verb;-complete={type}; & Type of command-line completion used.  See |\texttt{:h :command-completion}| for the list of possible values. \\
				\verb;-bar; & The command can be followed by \texttt{|} and another command, or \texttt{"} and a comment. \\
