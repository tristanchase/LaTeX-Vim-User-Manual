...
The option \verb!'fileformats'! contains the various formats that will be tried when a new file is edited.
...
To check for this format anyway, add "\verb!mac!" to \verb!'fileformats'!:
...
"\verb!++ff!" is used for \verb!'fileformat'!.
...
You can use the \verb!'fileformat'! option to convert from one file format to another.
...
To disable the encryption of a file, set the \verb!'key'! option to an empty string:
...
Setting the \verb!'key'! option to enable encryption is not a good idea, because the password appears in the clear.
...
This sets the \verb!'binary'! option.
The effect of this is that unexpected side effects are turned off.
For example, \verb!'textwidth'! is set to zero, to avoid automatic formatting of lines.
...
To get some overview switch the \verb!'wrap'! option off:
...
This takes into account how \verb!'fileformat'! changes the number of bytes that a line break uses.
